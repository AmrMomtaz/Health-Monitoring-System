...
def query(date1, date2):
    ...
    for tup in years:
        ...
        final_df = pd.concat([final_df, sql('/home/hdoop/Desktop/Spark/MapReduceOutput/year.parquet', ts1, ts2)])
    # Querying months
    for tup in months:
        ts1 = tup[0] + '-01 00:00:00'
        ts2 = tup[1] + '-01 00:00:00'
        final_df = pd.concat([final_df, sql('/home/hdoop/Desktop/Spark/MapReduceOutput/month.parquet', ts1, ts2)])
    # Querying days
    for tup in days:
        ts1 = tup[0] + ' 00:00:00'
        ts2 = tup[1] + ' 00:00:00'
        final_df = pd.concat([final_df, sql('/home/hdoop/Desktop/Spark/MapReduceOutput/day.parquet', ts1, ts2)])
    # Querying hours
    for tup in hours:
        ts1 = tup[0] + ':00:00'
        ts2 = tup[1] + ':00:00'
        final_df = pd.concat([final_df, sql('/home/hdoop/Desktop/Spark/MapReduceOutput/hour.parquet', ts1, ts2)])
    # Querying minutes
    for tup in minutes:
        ts1 = tup[0] + ':00'
        ts2 = tup[1] + ':00'
        final_df = pd.concat([final_df, sql('/home/hdoop/Desktop/Spark/MapReduceOutput/minute.parquet', ts1, ts2)])
...
f = open("/home/hdoop/Desktop/Spark/realTimeView.txt")
state = int(f.read(1))
f.close()
...
FROM '/home/hdoop/Desktop/Spark/SparkOutput{state}/*.parquet' as d
